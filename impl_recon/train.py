    ...
    for i in range(num_iters):
        ...
        if (i + 1) % eval_every_x_steps == 0:
            ...
            if num_const_train_dsc == max_num_const_train_dsc:
                print(f"Reached stopping critertion after {i + 1} steps. " f"Optimization has converged.")
                break
            prev_train_dsc = dsc
...
def main():
    ...
    if train_folder is not None:
        params["model_name"] = params["model_name"] + "_" + Path(train_folder).name
    model_dir = model_basedir / params["model_name"] if params["model_name"] is not None else None

    # If not None, this will override all behavior so it uses all files from this folder.
    ...
    if model_dir is not None:
        if model_dir.exists():
            pass
            # raise ValueError("Model directory already exists. Exiting to prevent accidental " f"overwriting.\n{model_dir}")
        else:
            model_dir.mkdir()
        ...
        checkpoint_writer = io_utils.RollingCheckpointWriter(model_dir, "checkpoint", max_num_checkpoints, "pth")
    else:
