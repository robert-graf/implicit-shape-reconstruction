...
def read_yaml_config(config_file_path: Union[Path, str], default_config_file_path: Optional[Union[Path, str]] = None) -> Dict:
...
def read_train_config(config_file_path: Union[Path, str], default_config_file_path: Optional[Union[Path, str]] = None) -> Dict:
...
def parse_config_train() -> Tuple[Dict, Path]:
    ...
    parser.add_argument("-f", "--train_folder", type=str, default=None)
...
def parse_config_eval() -> Tuple[Dict, Path]:
    ...
    config file.
    """
    parser = argparse.ArgumentParser(description="Evaluate a neural network.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-p", "--path_file", type=str)
    parser.add_argument("-m", "--model_name", type=str)
    parser.add_argument("-c", "--eval_config", type=str)
    parser.add_argument("-max", "--max", type=int, default=20)
    parser.add_argument("-min", "--min", type=int, default=26)
    args = parser.parse_args()
    ...
    #    raise ValueError(
    #        f'Cannot load stored model "{args.model_name}": it contains a different ' f'model name in the config: "{config["model_name"]}".'
    #    )
