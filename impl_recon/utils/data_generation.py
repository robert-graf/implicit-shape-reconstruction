...
def downsample(image: torch.Tensor, target_spatial_shape: List[int], do_threshold: bool) -> torch.Tensor:
    ...
    image_lowres = functional.interpolate(  # type: ignore[call-arg]
        image, size=target_spatial_shape, mode="trilinear", align_corners=False, recompute_scale_factor=False
    )
    if do_threshold:
...
def compute_thick_slices(volume: torch.Tensor, axis: int, step_size: int) -> torch.Tensor:
    ...
    """
    thick_slices = []
    max_num_slices_below = round(step_size / 2) - 1 if step_size % 2 == 0 else round(step_size / 2 - 0.5)
...
def select_fullres_subset(num_cases_total: int, fraction: float) -> List[int]:
    ...
    caseids = torch.multinomial(torch.ones(num_cases_total), num_cases_actual, False, generator=gen_subset).tolist()
    return caseids
...
def split_train_val_casenames(casenames: List[str], val_fraction: float, is_training: bool) -> List[str]:
...
class ReconNetDataset(data.Dataset):
    def __init__(
        ...
        if do_deterministic_sparsing:
            ...
            self.slice_start_ids = torch.randint(0, slice_step_size, [num_cases_original], generator=gen).tolist()
...
class OrthogonalSlices(data.Dataset):
    def __init__(self, labels_dir: Path, casenames: List[str], verbose: bool):
        ...
        # self.labes: List[(x,y,z)]
        image_sizes = [torch.tensor(label.shape) * spacing for label, spacing in zip(self.labels, self.spacings)]
        # self.image_sizes: List[Tensor(x,y,z)]
...
class ImplicitDataset(OrthogonalSlices):
    ...
    def __init__(
        ...
        gen = torch.Generator().manual_seed(SPRSF_START_MAIN_SEED)
        slice_start_ids = torch.randint(0, slice_step_size, [len(self.labels)], generator=gen).tolist()
        results = [
            sparsen_volume(vol, spacing, offset, slice_step_axis, slice_step_size, slice_start_id, use_thick_slices)
            for vol, spacing, offset, slice_start_id in zip(self.labels, self.spacings, self.offsets, slice_start_ids)
        ]
        ...
        if phase_type != PhaseType.INF:
            ...
            start_slice_ids = torch.randint(0, 2, [len(val_caseids)], generator=gen_val_sparsification).tolist()
            ...
            for caseid, start_slice_id in zip(val_caseids, start_slice_ids):
                self.labels_sparse[caseid], self.spacings_sparse[caseid], self.offsets_sparse[caseid] = sparsen_volume(
        ...
        self.num_points_per_example = (
            num_points_per_example_per_dim ** self.image_size.numel() if num_points_per_example_per_dim > 0 else -1
        )
...
def create_data_loader(params: Dict, phase_type: PhaseType, verbose: bool) -> data.DataLoader:
    ...
    dl = data.DataLoader(ds, batch_size, do_shuffle, num_workers=num_workers, drop_last=do_drop_last)
    return dl
