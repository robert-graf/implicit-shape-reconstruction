...
def save_nifti_file(image_data: np.ndarray, local_to_global: np.ndarray, target_filepath: Path):
    ...
    nib.save(img, str(target_filepath).replace("*", ""))
...
def find_single_file(source_dir: Path, pattern: str) -> Path:
    ...
    if len(matched_files) != 1:
        raise ValueError(
            "The source directory has to contain exactly one file that matches the " "pattern {}:\n{}".format(pattern, source_dir)
        )
...
class RollingCheckpointWriter:
    ...
    def __init__(self, base_dir: Path, base_filename: str, max_num_checkpoints: int, extension: str = "pth"):
    ...
    def write_rolling_checkpoint(self, model_state: dict, optimizer_state: dict, num_steps_trained: int, num_epochs_trained: int):
        ...
        target_filepath = self.base_dir / "{}_{}.{}".format(self.base_filename, num_steps_trained, self.extension)
...
def load_latest_checkpoint(
    ...
    if not paths:
        raise ValueError("Model directory for checkpoint loading does not contain any " "checkpoints:\n{}".format(base_dir))
    ...
    return state["model_state"], state["optimizer_state"], state["num_steps_trained"], state["num_epochs_trained"]
