...
def main():
    ...
    for i, batch in enumerate(ds_loader):
        ...
        if task_type == config_io.TaskType.AD:
            ...
            if labels_gt_sparse.shape[0] != 1:
                raise ValueError(f"Only batch size 1 is supported for AD, instead got " f"{labels_gt_sparse.shape[0]}.")
            assert isinstance(net, implicits.AutoDecoder)
            ...
            labels_pred = impl_utils.sample_latents(latents_batch, net, target_spatial_shape, spacings)
    ...
    if all_dice_metrics and asds and hd95s and max_distances:
        print(f"ASD: {np.mean(asds):.2f} +- {np.std(asds):.2f} in " f"[{np.min(asds):.2f}, {np.max(asds):.2f}]")
        print(f"HSD95: {np.mean(hd95s):.2f} +- {np.std(hd95s):.2f} in " f"[{np.min(hd95s):.2f}, {np.max(hd95s):.2f}]")
        print(
