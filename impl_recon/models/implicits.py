...
class OccupancyPredictor(torch.nn.Module):
    def __init__(self, latent_dim: int, spatial_dim: int, num_layers: int, layers_with_coords: List[int]):
        def block(num_ch_in: int, num_ch_out: int):
            return torch.nn.Sequential(torch.nn.Linear(num_ch_in, num_ch_out), torch.nn.ReLU(True))
        ...
        self.res_layers = torch.nn.ModuleList([block(in_channels[i], latent_dim) for i in range(num_layers - 1)])
...
class AutoDecoder(torch.nn.Module):
    ...
    def __init__(
        self, lat_dim: int, spatial_dim: int, image_size: torch.Tensor, occnet_num_layers: int, occnet_layers_with_coords: List[int]
    ):
        """
        ...
        self.occp_pred = OccupancyPredictor(lat_dim, spatial_dim, occnet_num_layers, occnet_layers_with_coords)
    ...
    def forward(self, latents: torch.Tensor, coordinates: torch.Tensor) -> torch.Tensor:
        ...
        latents = latents.expand(-1, coordinates.shape[1], coordinates.shape[2], coordinates.shape[3], -1)
...
class ReconNet(torch.nn.Module):
    ...
    def __init__(self):
        def consecutive_conv(in_channels: int, out_channels: int, do_half_intermediate: bool = False):
        ...
        self.conv_rest_u_32 = consecutive_conv_up(num_channels * 8 + num_channels * 4, num_channels * 4)
        self.conv_rest_u_64 = consecutive_conv_up(num_channels * 4 + num_channels * 2, num_channels * 2)
        self.conv_rest_u_128 = consecutive_conv_up(num_channels * 2 + num_channels, num_channels)
